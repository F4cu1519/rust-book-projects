...
// ============================================================
fn main() {
    println!("=== 1. Spawn básico (el hilo creado puede cortarse) ===");
    ejemplo_spawn_basico();

    println!("\n=== 2. Join después del for (hilos intercalados) ===");
    ejemplo_join();

    println!("\n=== 3. Join antes del for (ejecución secuencial) ===");
    ejemplo_join_antes_del_for();

    println!("\n=== 4. Move closure (ownership transferido al hilo) ===");
    ejemplo_move_closure();

    println!("\n=== 5. Retorno de valor desde hilo ===");
    let valor = ejemplo_retorno_desde_hilo();
    println!("El hilo retornó: {valor}");

    println!("\n=== 6. Múltiples hilos con Arc<Mutex<T>> ===");
    let total = ejemplo_multiples_hilos_con_mutex();
    println!("Contador final tras 10 hilos: {total}");
}
